...
/**
 * 提供通用唯一识别码（universally unique identifier）（UUID）实现，UUID表示一个128位的值。<br> 此类拷贝自java.util.UUID，用于生成不带-的UUID字符串
 *
 * <p>
 ...
 * <p>
 * version 字段保存描述此 UUID 类型的值。有 4 种不同的基本 UUID 类型：基于时间的 UUID、DCE 安全 UUID、基于名称的 UUID 和随机生成的 UUID。<br> 这些类型的 version 值分别为
 * 1、2、3 和 4。
 *
 * @since 4.1.11
 */
